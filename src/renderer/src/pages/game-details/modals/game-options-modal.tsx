...
export function GameOptionsModal({
  ...
  // Accountless fork: the legacy cloud panel is the local backup UI, so it is
  // always accessible — no sign-in or paywall redirect.
  const cloudSaveAccessAction = ACCOUNTLESS
    ? "open"
    : getCloudSaveAccessAction(Boolean(userDetails), hasActiveSubscription);
